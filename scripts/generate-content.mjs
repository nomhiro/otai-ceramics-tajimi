 ...
 * 酒器Markdownを生成
 */
function generateSakewareMarkdown(type, data) {
  if (type === 'tokuri') {
    return `---
title: "${data.name}"
type: "tokuri"
code: "${data.code}"
images:
  - "/images/sake/tokuri_${data.code}.jpg"
capacity: "${data.capacity}"
inStock: true
---

${data.name}（${data.capacity}）。美濃焼の伝統的な徳利です。
`.trim();
  } else {
    return `---
title: "${data.name}"
...
code: "${data.code}"
images:
  - "/images/sake/choko_${data.code}.jpg"
inStock: true
---

${data.name}。美濃焼の伝統的な猪口です。
`.trim();
  }
}
 ...
 * ギャラリーMarkdownを生成
 */
function generateGalleryMarkdown(category, data) {
  if (category === 'jokki') {
    return `---
title: "${data.name}"
category: "jokki"
code: "${data.code}"
image: "/images/gallery/jokki_${data.code}.jpg"
description: "${data.description || ''}"
---

${data.maker}の${data.name}。
`.trim();
  } else {
    return `---
title: "${data.name}"
category: "shioyuu"
code: "${data.code}"
image: "/images/gallery/shioyuu_${data.code}.jpg"
artist: "${data.artist}"
description: "${data.description || ''}"
---

先代 ${data.artist}の塩釉作品。平成13年「美濃の民芸展」出品作品。
`.trim();
  }
}
